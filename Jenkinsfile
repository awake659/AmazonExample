...
pipeline {
  ...
  stages {
    stage('Build Dependencies') {
     ...
     stage('Build') {
         steps {
          sh 'aws cloudformation create-stack --template-url https://anthony-bucket-three.s3-eu-west-1.amazonaws.com/cloudformation.template --stack-name anthonyteststack --capabilities CAPABILITY_IAM --region ap-northeast-1\n'
